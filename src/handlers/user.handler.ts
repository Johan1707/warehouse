...
export const createUserHandler = factoryDB.createHandlers(async (c: Context) => {
    ...
    if(error) {
        return c.json({ message: t(error.message) }, error.status)
    }

    return c.json(user, 201 as StatusCode)
})

export const getAllUsersHandler = factoryDB.createHandlers(async (c: Context) => {
    const t = useTranslation(c)
...
export const getUserByIdHandler = factoryDB.createHandlers(async (c: Context) => {
    const t = useTranslation(c)
...
export const updateUserHandler = factoryDB.createHandlers(async (c: Context) => {
    const t = useTranslation(c)
    const id: IdParam = c.req.valid(('param') as never)
...
export const deleteUserHandler = factoryDB.createHandlers(async (c: Context) => {
    const t = useTranslation(c)
    const id: IdParam = c.req.valid(('param') as never)
